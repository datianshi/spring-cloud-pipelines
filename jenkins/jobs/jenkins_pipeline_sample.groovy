...
parsedRepos.each {
	...
	dsl.job("cms/${projectName}-test-env-deploy") {
	...
	if (rollbackStep) {
		dsl.job("cms/${projectName}-test-env-rollback-deploy") {
		...
		dsl.job("cms/${projectName}-test-env-rollback-test") {
	...
	if (stageStep) {
		dsl.job("cms/${projectName}-stage-env-deploy") {
		...
		dsl.job("cms/${projectName}-stage-env-test") {
	...
	dsl.job("cms/${projectName}-prod-env-deploy") {
	...
	dsl.job("cms/${projectName}-prod-env-complete") {
